...
window.confirmDelete = function (form) {
  form.addEventListener('submit', function (event) {
    ...
    sweetalert2__WEBPACK_IMPORTED_MODULE_0___default().fire(popupDeleteConfirmation).then(function (result) {
      if (result.isConfirmed) form.submit();
    });
  });
};
window.popupAction = function (popup, targetUrl, chosenMethod) {
  sweetalert2__WEBPACK_IMPORTED_MODULE_0___default().fire(popup).then(function (result) {
    if (!result.isConfirmed) return;
    var inputData = result.value;
